...
namespace Microsoft.Azure.NotificationHubs.DotNetCore.Tests
{
    public class NotificationContentTypeTest
    {
        ...
        public void NotificationContentTypeCorrespondsToApi()
        {
            ...
            Assert.Equal("application/octet-stream", new WindowsNotification("<toast><visual><binding template=\"ToastText01\"><text id=\"1\">bodyText</text></binding>  </visual></toast>", new Dictionary<string, string>() { { "X-WNS-Type", "wns/raw" } }).ContentType);
